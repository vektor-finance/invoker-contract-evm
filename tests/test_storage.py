...
def test_set_sender(storage, address):
    storage.writeAddress(KECCAK_MSGSENDER,address)
...
def test_set_value(storage, value):
    storage.writeUint256(KECCAK_BALANCE, value)
    assert storage.readUint256(KECCAK_BALANCE) == value

@given(test_bytes=strategy('bytes32'))
def test_set_bytes(storage, test_bytes):
    storage.write(KECCAK_BYTES, test_bytes)
