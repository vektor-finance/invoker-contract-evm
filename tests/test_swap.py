...
from brownie import Invoker, CSwap, CMove, accounts, Contract
...
def test_buy_dai(user, dai, WETH, uni_router):
    ...
    uni_router.swapExactETHForTokens(2700 * 1e18, path, user, deadline, {'from': user, 'value': 1e18})
...
@pytest.mark.require_network("mainnet-fork")
def test_swap_dai_usdc_via_delegate_invoker_individually(user, dai, usdc, cswap, cmove, WETH, uni_router, invoker):
    ...
    calldata_swap = cswap.swapExactTokensForTokens.encode_input(1000*1e18, 0, [dai.address, usdc.address])
